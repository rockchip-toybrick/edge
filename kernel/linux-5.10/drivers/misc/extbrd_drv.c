...
#define EXTBRD_DEBUG(s, ...) printk("Extbrd *** " s, ##__VA_ARGS__)

#define EXTBRD_ERROR(s, ...) printk("Extbrd *** " s, ##__VA_ARGS__)
...
#define DIGIT_TUBE_NUM		49
#define EXT_ITEM_MAX_NUM 	50
...
static int extbrd_probe(struct platform_device *pdev)
{
	...
	if (ddata->has_extgpios) {
		...
		for (i = 0; i < ddata->gpio_num; i++) {
			...
			if (ret != 0) {
				EXTBRD_ERROR("Request gpio : %d fail, i %d\n", ext_gpios[i], i);
				ret = -EIO;
				return ret;
			}
