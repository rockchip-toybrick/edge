...
#define RV1103B_CLK_DUMP(_id, _name, _iscru)	\
{						\
	.id = _id,				\
	.name = _name,				\
	.is_cru = _iscru,			\
...
static const struct rv1103b_clk_info clks_dump[] = {
	RV1103B_CLK_DUMP(PLL_GPLL, "gpll", true),
	RV1103B_CLK_DUMP(ACLK_PERI_SRC, "aclk_peri_src", true),
	RV1103B_CLK_DUMP(LSCLK_PERI_SRC, "lsclk_peri_src", true),
	RV1103B_CLK_DUMP(PCLK_PERI_ROOT, "pclk_peri_root", true),
	RV1103B_CLK_DUMP(PCLK_TOP_ROOT, "pclk_top_root", true),
	RV1103B_CLK_DUMP(LSCLK_PMU_ROOT, "lsclk_pmu_root", true),
};
#endif
...
#ifdef CONFIG_SPL_BUILD
        /* fix lsclk_prei div */
   	writel(BITS_WITH_WMASK(1, 0x1U, 9),
	       RV1103B_CRU_BASE + RV1103B_CLKSEL_CON(31));
        /* fix cpu div */
    	writel(BITS_WITH_WMASK(1, 0x7U, 13),
	       RV1103B_CRU_BASE + RV1103B_CLKSEL_CON(37));
	/* fix gpll postdiv1 */
	writel(BITS_WITH_WMASK(1, 0x7U, 12),
	       RV1103B_CRU_BASE + RV1103B_PLL_CON(24));
#endif
...
static int rv1103b_clk_ofdata_to_platdata(struct udevice *dev)
{
	struct rv1103b_clk_priv *priv = dev_get_priv(dev);
...
static int rv1103b_clk_bind(struct udevice *dev)
{
	...
	if (ret) {
		...
		sf_priv->sf_reset_offset = offsetof(struct rv1103b_cru,
						    peri_softrst_con[0]);
...
U_BOOT_DRIVER(rockchip_rv1103b_cru) = {
	.name		= "rockchip_rv1103b_cru",
	.id		= UCLASS_CLK,
	.of_match	= rv1103b_clk_ids,
	.priv_auto_alloc_size = sizeof(struct rv1103b_clk_priv),
	.ofdata_to_platdata = rv1103b_clk_ofdata_to_platdata,
	.ops		= &rv1103b_clk_ops,
...
int soc_clk_dump(void)
{
	...
	const struct rv1103b_clk_info *clk_dump;
	struct clk clk;
	...
	ret = uclass_get_device_by_driver(UCLASS_CLK,
					  DM_GET_DRIVER(rockchip_rv1103b_cru),
					  &cru_dev);
	if (ret) {
	...
	ret = (readl(&priv->cru->core_clksel_con[0]) &
	       CLK_CORE_SRC_SEL_MASK) >>
	      CLK_CORE_SRC_SEL_SHIFT;
	if (ret == CLK_CORE_SRC_SEL_PVTPLL)
